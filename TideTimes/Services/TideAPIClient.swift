...
class TideAPIClient {
    ...
    private func findNearestStation(latitude: Double, longitude: Double) async throws -> NOAAStation {
        ...
        if let cached = Self.cachedStations {
            ...
            let components = URLComponents(string: "https://api.tidesandcurrents.noaa.gov/mdapi/prod/webapi/stations.json?type=waterlevels&units=english")
        ...
        let nearestStations = stations
            .filter { station in
                // Filter for active stations with valid IDs
                !station.id.isEmpty && 
                station.id.count <= 7 && // Valid NOAA station IDs are typically 7 digits
                station.lat != 0 && 
                station.lng != 0
            }
        ...
        guard let nearestStation = nearestStations.first?.0 else {
            throw NSError(
                domain: "TideAPI",
                code: -1,
                userInfo: [NSLocalizedDescriptionKey: "No tide stations found within 100km of \(latitude), \(longitude). Please try a different location closer to the coast."]
            )
        }
        
        print("Found nearest station: \(nearestStation.name) (\(nearestStation.id)) - \(String(format: "%.2f", nearestStations.first!.1))km away")
